<template>
    <div>
                ...
                <el-table-column label="操作" width="160" align="center">
                    <template slot-scope="scope">
                        ...
                        <el-button
                            ...
                            icon="el-icon-delete"
                            class="red"
                            @click="deletePara(scope.$index, scope.row)"
                        >删除</el-button>
                    </template>
                </el-table-column>
            </el-table>
        ...
        <el-dialog :title="title" :visible.sync="editVisible" width="50%">
            <el-form ref="paraForm" :model="paraForm" :rules="rules" label-width="150px">
                <el-form-item label="名称" required prop="name">
                    <el-input v-model="paraForm.name" placeholder="请输入名称" class="handle-input"></el-input>
                </el-form-item>
                <el-form-item label="选择模板" required prop="templateId">
                    <el-select v-model="paraForm.templateId" no-match-text="请选择" placeholder="请选择">
                        <el-option
...
<script>
import { paraPage, add, update, deletePara, deleteParas } from '../../api/para';
...
export default {
    ...
    created() {
        if (this.$route.query && this.$route.query.templateId) {
            ...
            this.paraForm.templateId = this.query.templateId;
        } else {
            this.query.templateId = null;
            this.paraForm.templateId = null;
        }
    ...
    methods: {
        // 获取 easy-mock 的模拟数据
        getParaPage() {
            paraPage(this.query).then(res => {
                if (res.code == 200) {
                    if (res.data.list && res.data.list.length > 0) {
                        this.paraList = res.data.list;
                    } else {
                        this.paraList = [];
                    }
        ...
        resetForm() {
            this.$refs['paraForm'].resetFields();
            this.paraForm = { templateId: this.query.templateId };
        },
        //添加操作
        async add() {
            await add(this.paraForm).then(res => {
                if (res.code === 200) {
        ...
        //修改操作
        async update() {
            await update(this.paraForm).then(res => {
                if (res.code === 200) {
                    this.$message.success(`修改成功`);
                    this.editVisible = false;
                    this.$set(this.paraList, this.showIndex, this.paraForm);
                } else {
                    this.$message.error(res.message);
                }
            });
        ...
        // 删除操作
        deletePara(index, row) {
            // 二次确认删除
            this.$confirm('确定要删除吗？', '提示', {
                type: 'warning'
            })
                .then(() => {
                    deletePara(row.id).then(res => {
                        if (res.code === 200) {
        ...
        deleteParas() {
            if (this.multipleSelection && this.multipleSelection.length > 0) {
                let paraIds = this.multipleSelection.map(data => {
                    return data.id;
                });
                // 二次确认删除
                this.$confirm('确定要删除吗？', '提示', {
